 ...
 */
export function useApiData<K = any>(
    url: string,
    options?: UseApiDataOptions<K>,
) {
    const { apiRequest } = useApi<K>();
...
/**
 * Custom React hook for performing POST (or other operations) mutations to a specified API endpoint.
 *
 ...
 *
 * @example
 * const mutation = useApiMutation<MyResponseType, MyRequestType>('/api/resource');
 ...
 */
export function useApiMutation<R = any, D = any>(
    url: string,
    options?: UseMutationOptions<R, AxiosError, D> & {
    ...
    return useMutation<R, AxiosError, D>({
        mutationFn: (data?: D) => {
            return apiRequest(url, {
                data: data,
                method: options?.method || 'POST', // Usa 'POST' como valor predeterminado
            }) as Promise<R>;
        },
        ...options,
    });
