...
export function MembersSidebar({ serverId, className }: MembersSidebarProps) {
  ...
  const groupMembersByStatus = (membersList: typeof members) => {
    return {
      online: membersList.filter((member) => member.user.status === "online"),
      away: membersList.filter((member) => member.user.status === "away"),
      busy: membersList.filter((member) => member.user.status === "busy"),
      offline: membersList.filter((member) => member.user.status === "offline"),
    }
  }
  ...
  const renderMemberGroup = (title: string, membersList: typeof members, showRole = false) => {
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1">
                  ...
                  {showRole && member.role === "ADMIN" && <Crown className="h-3 w-3 text-yellow-500 flex-shrink-0" />}
                </div>
                <div className="text-xs text-zinc-500 truncate">{getStatusText(member.user.status)}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  const renderStatusGroup = (status: string, adminList: typeof members, memberList: typeof members) => {
  ...
  return (
    <div className={cn("w-60 h-full bg-zinc-800 flex flex-col border-l border-zinc-700", className)}>
      ...
      <ScrollArea className="flex-1 px-2 py-3">
        <div className="space-y-2">
          {/* Online Members */}
          {renderStatusGroup("online", adminsByStatus.online, membersByStatus.online)}

          {/* Away Members */}
          {renderStatusGroup("away", adminsByStatus.away, membersByStatus.away)}

          {/* Busy Members */}
          {renderStatusGroup("busy", adminsByStatus.busy, membersByStatus.busy)}

          {/* Offline Members */}
          {renderStatusGroup("offline", adminsByStatus.offline, membersByStatus.offline)}
        </div>
      </ScrollArea>
    </div>
  )
